...
public class AnketRepository : IAnketRepository
{
    ...
    public bool AnketeSoruEkle(int anketId, int soruId)
    {
        var anket = InMemoryStore.Anketler.FirstOrDefault(a => a.Id == anketId);
        if (anket == null) return false;

        var soru = InMemoryStore.Sorular.FirstOrDefault(s => s.Id == soruId);
        if (soru == null) return false;

        if (InMemoryStore.AnketSorular.Any(x => x.AnketId == anketId && x.SoruId == soruId))
            return false; // Zaten ekli

        var siradakiSira = InMemoryStore.AnketSorular
            .Where(x => x.AnketId == anketId)
            .Select(x => x.SiraNo)
            .DefaultIfEmpty(0)
            .Max() + 1;

        InMemoryStore.AnketSorular.Add(new AnketSoru
        {
            AnketId = anketId,
            SoruId = soruId,
            SiraNo = siradakiSira
    ...
    public bool AnkettenSoruKaldir(int anketId, int soruId)
    {
        var anketSoru = InMemoryStore.AnketSorular
            .FirstOrDefault(x => x.AnketId == anketId && x.SoruId == soruId);

        if (anketSoru == null) return false;
