...
public class AnketService : IAnketService
{
    ...
    public async Task<ServiceResult> AnketeSoruEkleAsync(int anketId, int soruId, CancellationToken cancellationToken = default)
    {
        var basarili = _anketRepository.AnketeSoruEkle(anketId, soruId);
        if (!basarili)
            return ServiceResult.Fail("Soru zaten bu ankete ekli veya anket/soru bulunamadı.", StatusCodes.Status400BadRequest);

        await InvalidateAnketCacheAsync(cancellationToken);
        return ServiceResult.Success(StatusCodes.Status200OK);
    }

    public async Task<ServiceResult> AnkettenSoruKaldirAsync(int anketId, int soruId, CancellationToken cancellationToken = default)
    {
        var basarili = _anketRepository.AnkettenSoruKaldir(anketId, soruId);
        if (!basarili)
            return ServiceResult.Fail("Bu soru ankette bulunamadı.", StatusCodes.Status404NotFound);
