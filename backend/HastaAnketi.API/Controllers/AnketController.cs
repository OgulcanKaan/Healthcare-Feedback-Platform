...
public class AnketController : ControllerBase
{
    ...
    [HttpPost("{id:int}/sorular/{soruId:int}")]
    [Authorize(Roles = "Yonetici")]
    public async Task<IActionResult> AnketeSoruEkle(int id, int soruId, CancellationToken cancellationToken)
    {
        var result = await _anketService.AnketeSoruEkleAsync(id, soruId, cancellationToken);
        return this.ToActionResult(result);
    }

    [HttpDelete("{id:int}/sorular/{soruId:int}")]
    [Authorize(Roles = "Yonetici")]
    public async Task<IActionResult> AnkettenSoruKaldir(int id, int soruId, CancellationToken cancellationToken)
    {
        var result = await _anketService.AnkettenSoruKaldirAsync(id, soruId, cancellationToken);
        return this.ToActionResult(result);
    }
}
